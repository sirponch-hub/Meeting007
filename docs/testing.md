...
- With the verified Russian model installed, start speaking immediately after pressing Start and verify the first phrase is not silently lost after the model finishes preparing.
- During fast Russian speech, verify the live transcript updates the same partial line instead of adding repeated copies of the same phrase.
