...
- Handoff notes: The first real wiring emits final speech-window segments through the existing VAD/STT path. Because the first accepted model install did not include tokenizer files, Settings should now treat that CoreML-only install as invalid and require reinstall before real WhisperKit transcription can work. Manual QA with real Russian mic speech on Apple Silicon is required before merging to `main`; ordinary checks use fakes and do not require private audio fixtures.
