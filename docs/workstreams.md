...
- Status: Done — dogfood concurrency and full-spool finalization fix verified and merged to `main`
...
- Scope: Add backend-only rolling audio buffer, rolling-window decode contract, LocalAgreement-style transcript stabilizer, append-only committed transcript invariant, seam-local rolling hypothesis filtering so overlap/prompt echo is not shown as repeated live text, punctuation/case-tolerant overlap matching for shifted Russian rolling windows, conservative short-overlap handling so plausible repeated speech is not deleted, best-visible-draft finalization for the manual rolling smoke command, smoke-only QA tail reconciliation for end-of-audio loss detection, optional expected-text quality scoring for local smoke runs, fake-decoder checks showing partial text can appear before the current VAD final-chunk path, a real WhisperKit rolling-window adapter that can decode rolling windows with committed transcript prompt context, and a manual smoke command for rolling-vs-5s-batch comparison on a local Russian fixture.
...
- Verification: `swift run Meeting007CoreChecks` passed; `swift run Meeting007WhisperKitChecks` passed; `swift build --product Meeting007RollingWhisperKitSmoke` passed with sandbox escalation; `swift build --product Meeting007App` passed with sandbox escalation for Swift/clang cache access; local private `ex.wav` quality smoke with external expected text returned Rolling+QA-tail WER 0.071 / CER 0.013 / Recall 0.929 marked `PASS`, while raw Batch stayed WER 0.257 / CER 0.173 / Recall 0.800 marked `REVIEW`.
- Gates: Research agent identified current VAD/WhisperKit path as batch, not real-time; architecture/QA agent recommended a backend-only rolling-buffer spike with fake decoder and strict privacy boundaries; follow-up BA/Architecture/Developer agents recommended seam-local deduplication rather than global phrase removal after the user observed repeated rolling output; follow-up architecture/QA agent required committed transcript to be append-only by content after user smoke output showed the accepted beginning being dropped; user acceptance pending.
- Subagents: Research `019ecc5a-4906-7240-8764-bdc6b68e5402`; Architecture/QA `019ecc5e-48db-7111-be9b-bbdd11b19fb3`; BA `019ecf1b-f46a-7db0-871e-e954eb07d5fe`; Rolling architecture/QA `019ecf1c-0e60-7550-8234-68dc86749f15`; Developer/TDD `019ecf1d-4cca-7072-be1f-a64768da7b93`; Append-only rolling QA `019ecf21-9bd8-7ad2-87c8-2b9dfa16bae1`; Developer/TDD review `019ecf23-20cb-7c82-b606-a9251b1bb800`; Smoke tail reconciliation QA `019ecf55-dd36-76d2-b97d-4a095aec8497`.
- TDD/BDD evidence: Added checks for bounded rolling buffer retention, Stop cleanup, wrong-session rejection, earlier partial output than VAD final chunks, LocalAgreement stable-prefix commits, rejected incompatible longer committed prefixes, shifted-window committed-beginning preservation, best-visible-draft rolling finalization, smoke-only tail reconciliation appending missing endings without duplicates or noisy middle replacement, transcript quality scoring normalization/WER/CER, seam-local prompt echo removal, suffix/prefix rolling overlap merging, punctuation/case-tolerant overlap merging, real repeated Russian phrase preservation, short repeated phrase preservation at rolling boundaries, repeated partial replacement behavior, verified-model rolling adapter construction, missing-model no-engine behavior, rolling prompt/window forwarding, and rolling runtime failure reporting.
...
- Handoff notes: This spike intentionally does not wire into `Meeting007App`. WhisperKit context is supported through tokenizer-encoded `promptTokens`. Run the manual benchmark with `swift run Meeting007RollingWhisperKitSmoke /path/to/russian-audio.wav`; add `--expected-text-file /path/to/expected.txt` or `--expected-text '...'` for aggregate quality scoring. The command prints only changed rolling live states, promotes the best visible rolling draft for smoke final output, and compares it with 5-second batch output without saving audio or transcript files. Production recording still needs a separate decision on final-pass transcription or segment-level stabilization before this is wired into the main app.
