...
- Scope: Add verified model directory provider in core, wire `Meeting007App` to `Meeting007WhisperKit`, add a production WhisperKit pipeline factory, build the WhisperKit engine only from a verified local model path, prepare and cache one WhisperKit runtime for the active session, preserve `download: false`, remove production fake STT default, install/verify required tokenizer files, surface runtime failures instead of silently returning empty transcripts, keep fake STT for explicit tests/dev injection, and update transcript surface copy from preview/future-slice language to live local transcription language.
...
- Handoff notes: The first real wiring emits final speech-window segments through the existing VAD/STT path. The installed model now includes tokenizer files, and the local engine prepares/caches the WhisperKit runtime at transcription start instead of rebuilding it per speech chunk. Manual QA with real Russian mic speech on Apple Silicon is required before merging to `main`; ordinary checks use fakes and do not require private audio fixtures.
