...
- Scope: Add verified model directory provider in core, wire `Meeting007App` to `Meeting007WhisperKit`, add a production WhisperKit pipeline factory, build the WhisperKit engine only from a verified local model path, preserve `download: false`, remove production fake STT default, keep fake STT for explicit tests/dev injection, and update transcript surface copy from preview/future-slice language to live local transcription language.
- Out of scope: WhisperKit latency optimization/prewarm caching, partial-rich word streaming, Russian quality benchmark thresholds, system-audio transcription, diarization, raw audio retention, REST/MCP changes, model repair/delete UX, cloud STT, telemetry, and hosted backend.
...
- TDD/BDD evidence: Added core checks for verified model directory path and corrupt-file rejection; added WhisperKit checks for verified-directory engine construction, missing-directory no-engine/no-download behavior, production pipeline not emitting fake text when model is missing, Russian defaults, transcript segment mapping, and no audio artifact creation.
- Open decisions: Real-world Russian accuracy threshold, live latency target after SDK load, whether to add session prewarm/cached WhisperKit engine despite Swift 6 isolation constraints, and whether recording should show a Settings shortcut when the model is missing.
- Handoff notes: The first real wiring emits final speech-window segments through the existing VAD/STT path. Manual QA with real Russian mic speech on Apple Silicon is required before merging to `main`; ordinary checks use fakes and do not require private audio fixtures.
