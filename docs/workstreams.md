...
- Scope: Add backend-only rolling audio buffer, rolling-window decode contract, LocalAgreement-style transcript stabilizer, append-only committed transcript invariant, seam-local rolling hypothesis filtering so overlap/prompt echo is not shown as repeated live text, punctuation/case-tolerant overlap matching for shifted Russian rolling windows, conservative short-overlap handling so plausible repeated speech is not deleted, best-visible-draft finalization for the manual rolling smoke command, fake-decoder checks showing partial text can appear before the current VAD final-chunk path, a real WhisperKit rolling-window adapter that can decode rolling windows with committed transcript prompt context, and a manual smoke command for rolling-vs-5s-batch comparison on a local Russian fixture.
